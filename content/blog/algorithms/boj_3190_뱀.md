...
## 접근법
주어진 조건만 맞게 구현하면 되는 시뮬레이션 문제이다.

덱 자료구조를 사용하여 뱀의 머리를 덱의 마지막에 저장하고, 꼬리를 처음 부분에 저장했다.

예를 들어 사과를 먹었을 경우, 머리 부분 위치를 덱의 마지막에 갱신하면 된다.

반대로 사과를 먹지 못 했을 경우, 꼬리에 해당하는 덱의 첫 부분을 제거하고, 머리 부분을 이동 및 갱신해주면 된다.

이 방법으로 벽이나 몸통에 부딪힐 때까지 반복수행.
